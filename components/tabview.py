...
class Tabview(ctk.CTkFrame):
    def __init__(self, parent, show_page):
        ...
        self.db_btn = ctk.CTkButton(
            tabFrame, text="Database", font=font, width=150, height=50,
            corner_radius=15, border_width=2, cursor="hand2",
            border_color="darkgrey", fg_color="#E0E0E0",
            hover_color="white", text_color="#F01382",
            command=lambda: self.show_tab("db")
        )
        self.db_btn.pack(side="left", fill="y", padx=10)

        self.calibrate_btn = ctk.CTkButton(
            tabFrame, text="Calibrate Lidar", font=font, width=150, height=50,
            corner_radius=15, border_width=2, cursor="hand2",
            border_color="darkgrey", fg_color="#E0E0E0",
            hover_color="white", text_color="#F01382",
        ...
        back = ctk.CTkButton(
            tabFrame, image=close, text="", width=40, height=40,
            font=font, fg_color="transparent", hover=False,
            text_color="#F01382", cursor="hand2",
        ...
        # (Database UI code as-is…)
        ctk.CTkLabel(self.db_tab, text="Address", font=font, text_color="#F01382").pack(pady=(80, 5))
        self.addr = ctk.CTkEntry(self.db_tab, width=400, height=40, font=font,
                                 placeholder_text="192.168.1.1", justify="center")
        self.addr.pack(pady=5)

        ctk.CTkLabel(self.db_tab, text="Username", font=font, text_color="#F01382").pack(pady=5)
        self.usn = ctk.CTkEntry(self.db_tab, width=400, height=40, font=font,
                                placeholder_text="root", justify="center")
        self.usn.pack(pady=5)

        ctk.CTkLabel(self.db_tab, text="Password", font=font, text_color="#F01382").pack(pady=5)
        self.passwd = ctk.CTkEntry(self.db_tab, width=400, height=40, font=font,
        ...
        self.port = ctk.CTkEntry(self.db_tab, width=400, height=40, font=font,
                                 placeholder_text="3306", justify="center")
        self.port.pack(pady=5)

        ctk.CTkLabel(self.db_tab, text="Database Name", font=font, text_color="#F01382").pack(pady=5)
        self.db_name = ctk.CTkEntry(self.db_tab, width=400, height=40, font=font,
                                    placeholder_text="LidarDB", justify="center")
        self.db_name.pack(pady=(5, 0))

        # Set data ke fields
        entries = [(self.addr, address_val), (self.usn, username_val),
                   (self.passwd, password_val), (self.port, port_val),
                   (self.db_name, dbname_val)]
        for entry, value in entries:
            if value:
                entry.insert(0, value)

        btn_area = ctk.CTkFrame(self.db_tab, fg_color="white")
        btn_area.pack(padx=20, pady=20)

        ctk.CTkButton(
            btn_area, text="Cancel", font=font, width=200, height=50,
            corner_radius=20, fg_color="red", hover_color="#ff4d4d",
            cursor="hand2", command=lambda: self.back_to_home(show_page)
        ).pack(fill="x", pady=10)

        ctk.CTkButton(
            btn_area, text="Save", font=font, width=200, height=50,
        ...
        # Tombol Show/Hide Viewer
        show_btn = ctk.CTkButton(
            self.calibrate_tab,
            text="Toggle 3D Viewer",
            fg_color="#F01382",
            font=fontSmall,
            hover=False,
            cursor="hand2",
            command=self.toggle_viewer_window
        )
        show_btn.pack(pady=10)

        # Konten calibrate (panel kanan)
        right.CalibrateTabContent(self.calibrate_tab, show_page=show_page).pack(
    ...
    # ============================================================
    # VIEWER WINDOW VISIBILITY CONTROL
    # ============================================================
